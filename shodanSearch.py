...
if __name__ == '__main__':
	...
	try:
		...
		if perform_query.upper() == 'Y':
			print('[+] Info: Fetching results from {0} pages. Results will be written to {1}. Shodan.io API calls are time throttled 1/sec.'.format(pages, args.file))
			for x in range(0,pages):
				...
				try:
					...
					print('[-] Error: Cannot open {0}'.format(args.file))
					sys.exit()
			print('[+] Info: Finished.\n')
		else:
